...
galton <- read.csv(file.path(.get_course_path(),
	"C11_Stats_02", "Introduction", "galton.csv"))
gch <- galton$child
gpa <- galton$parent
...
if  (isTRUE((Sys.getenv("RSTUDIO") == "1")&&(nchar(maniflg)>0 ))) {
  ...
  fname <- file.path(.get_course_path(),
  	"C11_Stats_02","Least_Squares_Estimation","slopedemo.R")
} else {
  fname <- file.path(.get_course_path(), 
  	"C11_Stats_02","Least_Squares_Estimation","slopedemo_no_mani.R")  
}
file.edit(fname)
fname2 <- file.path(.get_course_path(),
	"C11_Stats_02","Least_Squares_Estimation","finalplot.R")
